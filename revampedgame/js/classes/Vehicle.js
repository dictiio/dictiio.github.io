...
class Vehicle {
	constructor({ lane, x, width, height, vx = -1 }) {
		this.lane = lane;
		this.x = x;
		this.vx = vx;
		this.width = width;
		...
		if (this.lane.laneType == "road") {
			this.image =
				images.vehicles.car[Math.random() > 0.5 ? "one" : "two"];
		}
	}
	...
	checkCollision(player) {
		if (this.lane.laneType == "water") {
			...
			if (
				middleX > this.x &&
				middleX < this.x + this.width &&
				middleY > this.lane.y &&
				middleY < this.lane.y + this.height
			) {
				player.vx = this.vx;
